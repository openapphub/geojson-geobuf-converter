...
const upload = multer({
  ...
  fileFilter(req, file, cb) {
    ...
    if (
      ...
      cb(
        new Error('Only .json, .geojson, .pbf file formats are supported!'),
        false,
      );
    }
  },
  ...
  return new Promise((resolve, reject) => {
    fs.readFile(inputPath, 'utf8', function (err, data) {
      ...
      try {
        ...
        const buffer = geobuf.encode(geojsonObj, new Pbf());

        fs.writeFile(outputPath, buffer, function (error) {
          if (error) {
            reject(
              new Error(`Failed to write compressed file: ${error.message}`),
            );
          } else {
            resolve(outputPath);
          }
    ...
    fs.readFile(inputPath, function (err, buffer) {
      ...
      try {
        ...
        fs.writeFile(outputPath, jsonString, 'utf8', function (error) {
          if (error) {
            reject(
              new Error(`Failed to write decompressed file: ${error.message}`),
            );
          } else {
            resolve(outputPath);
          }
...
app.post('/api/compress', upload.single('file'), async (req, res) => {
  try {
    if (!req.file) {
      return res
        .status(400)
        .json({ error: 'Please select a file to compress' });
    }
...
app.post('/api/decompress', upload.single('file'), async (req, res) => {
  try {
    if (!req.file) {
      return res
        .status(400)
        .json({ error: 'Please select a file to decompress' });
    }
